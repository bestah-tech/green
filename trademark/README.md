...
| 3 | 모듈 5 최소판: 문구 자산 + 통지서 초안 + HWPX (→ MVP) | ✅ 완료 — 테스트 대기 |
| 4 | 모듈 2: 칭호 변형 엔진 + 검색식 조립 + 승인 2 | ✅ 완료 — 테스트 대기 |
| 5 | 모듈 3: 사이드패널 검색 에이전트 | 🔶 1차 완료 (수집·후보 저장·유사도 평가·승인 2 검색식 실행 기록) — 자동 실행은 내부망 확인 후 |
...
1. 승인 1이 확정된 출원건에서 **2. 검색식 작성** 탭 클릭 → "승인 1 확정" 배지 확인
2. **① 검색 용어 추출** → 용어 표(core/support)가 나오는지 확인 (목 모드 가능) → 필요 시 수정
3. **② 칭호 변형·확장 생성** → 변형 표가 나오는지 확인 → 쓸 변형만 체크
4. **③ 검색식 조립·검증** → 검색식 목록(핵심·보조·결합)과 검증 배지 확인 — 조립은 LLM 없이 코드가 `config/query_syntax.json` 문법으로 수행
5. 검색식을 직접 수정해 보고 (괄호를 지우면 "검증 실패"로 바뀌는지) → **승인 2** → 버전 이력에 v1 생성 확인
6. 검색 에이전트 사이드패널을 열어 **승인 2 검색식** 목록이 보이는지, [복사] 시 실행 횟수가 올라가는지 확인
